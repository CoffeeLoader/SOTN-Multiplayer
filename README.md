## Conheça o meu Canal no [Youtube](https://www.youtube.com/channel/UClAmzmIgj22vX8lia9-om-w)

[Português do Brasil](https://github.com/CoffeeLoader/SOTN-Multiplayer) |
[English](https://github.com/CoffeeLoader/SOTN-Multiplayer/blob/main/README%20EN.md) |
[Español](https://github.com/CoffeeLoader/SOTN-Multiplayer/blob/main/README%20ES.md)

---

## [Pagina de Download](https://github.com/CoffeeLoader/SOTN-Multiplayer/releases)

## Começo da Pagina
Nesse repositorio eu vou estar disponibilizando um codigo único de Gameshark que permite ter um Modo Multiplayer bem simples jogando com o Familiar. Todos os codigos aqui são completamente gratuitos e foram feitos do zero com ajuda de ferramentas como o Cheat Engine, qualquer semelhança com os endereços de outros usuarios é pura coincidencia.

### Emulador de PlayStation 1 que eu recomendo:
* https://github.com/stenzek/duckstation - Duckstation

# Requisitos
* Qualquer Sistema que rode PS1 e que dê suporte ao Gameshark.
* Um Controle conectado na Porta 2, no caso de um emulador, um controle configurado na Porta 2.
* Um Backup do seu Game. (SLUS-00067)

# Recursos Multiplayer (v1.2)

* Todos os Familiars já Desbloqueados na Entrada do Castelo.
* Altere a Posição do Familiar usando DPAD.
* Tenha controle dos Familiars.
* Melhorias relacionadas ao posicionamento do Familiar na Tela.
* Troque de Familiar no Inventário.
* Habilidades para Bat.
* Habilidades para Ghost.
* Habilidades para Faerie.
* Habilidades para Demon.
* Habilidades para Sword.
* Habilidades para Pixie.
* Habilidades para Nose Devil.
* 5 Easter Eggs/Segredos.

# Correções Úteis (Opcional)

* Remove as Bordas Pretas, mas pode deixar visivil alguns astefatos.
* Faz correções no Invetario do Alucard, nas opções de video do Duckstation coloque (Razão de Aspecto: 4:3 e Corte: Todas as Bordas).
* Abre o alçapão na entrada do Castelo Normal e Invertido, jogando com o Alucard é necessario ter a reliquia Leap Stone pra que o alçapão se abra.
* Faz com que a Wing Smash seja Infinita.
* Força Modo de Audio Stereo.
* Corrige os nomes Japoneses dos Familiars Pixie e NDevil, esse codigo tem condições pra evitar problemas em versões traduzidas do game.
* Permite o P1 de habilitar a Pixie e Nose Devil, ative a reliquia da Faerie ou Demon e aperte Select

# Modo Hard + Familiar Cloak (Opcional)
## Modo Hard
(Comece um Novo Jogo e escreva HARDMODE)
* Limite de 200 HP, 50 MP e 50 Hearts.
* Status Extremamente Baixos. (STR:1 CON:1 INT:1 LCK:1)
* Envenenamento Infinito, reduz danos e defesa além de não permitir regeneração de MP.
* Alterações em Glitchs valido apenas para o Modo Hard.
* Proibição das habilidades Soul Steal e Shield Rod + Alucard Shield.
* Esquiva Estendida.
* Pequenas alterações em Musicas.
* Comece com a Familiar Cloak no Inventário.

## Familiar Cloak
(Compre no Bibliotecário ou comece um Novo Jogo usando o nome FAMILIAR, substituí a Reverse Cloak)
* Traz habilidades especiais dependendo do Familiar que estiver jogando.
* O interior da capa muda de cor dependendo do Familiar que estiver jogando.
* Quando o Familiar chega no LV 99, você ganha essa habilidade de forma permanente, deis de que esteja usando a Capa.
* Embora foi pensado para o Modo Hard, também funciona no jogo normal.

# Problemas Conhecidos

* Há um Bug do proprio jogo que faz com que a Ponte na Entrada da Abandoned Mine se quebre ao apertar Triangulo no Segundo Controle, algo semelhante também pode acontecer em outras areas do jogo.
* Não é possivel abrir os portôes da Abandoned Mine, então eles sempre estaram abertos.
* Não consegui dar um bom suporte ao Bat e Ghost, então eles não são os mais recomendados pra se jogar.

# Controles

## Bat #1

![Bat](https://user-images.githubusercontent.com/36766041/180564550-50bb9ca8-5a9b-4f56-9cc3-67e5eaf6d7e5.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Vira automaticamente pra Esquerda e Direita
* Ataca ao colidir com os elementos
* Familiar Cloak: Dark Metamorphosis
* Tem ganho de Dano apenas no LV 99
* Circulo no Inventário pra selecionar o Bat

## Ghost #2

![Ghost](https://user-images.githubusercontent.com/36766041/180564764-66284fa2-17cd-4fd8-9b36-3880f652bde3.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Vira automaticamente pra Esquerda e Direita
* Quadrado: Se feito próximo de um inimigo ele ataca, mas se não manter o botão pressionado você perde o controle do Familiar
* L1: Se precionado no momento certo pode diminuir o tempo necessário para o ataque, mas se feito errado o ataque é cancelado
* Familiar Cloak: Resistência as Trevas
* Mudança das Habilidades e dos Danos de forma Original
* Quadrado no Inventário pra selecionar o Ghost

## Demon #3

![Demon](https://user-images.githubusercontent.com/36766041/180564894-b98e66c2-c84b-416d-897d-533b9f28e10e.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Triangulo + DPAD Esquerdo e Direito: Muda o Lado em que o Familiar está olhando
* L1: Ataque Horizontal Comum
* L2: Ataque Rapido com a Lança
* R1: Ataque Special Aleatório (LV 10 ou Superior)
* R2: Ataque Comum com a Lança
* Quadrado: Ataque Horizontal de Fogo
* X: Ataque Horizontal de Água/Gelo
* Circulo: Ataque Horizontal de Raio
* Familiar Cloak: Poção de INT
* Tem ganhos de Dano nos Níveis 20, 40, 60, 80.
* Select no Inventário pra selecionar o Demon

## Faerie #4

![Faerie](https://user-images.githubusercontent.com/36766041/180564939-aebc324a-d33b-4b19-8347-04ccc761b22d.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Vira automaticamente pra Esquerda e Direita
* Quadrado: Usa Potion
* X: Usa Antivenom
* Circulo: Usa Hammer
* Triangulo: Usa Uncurse
* X + Triangulo: Usa Life Apple
* R1: Resistencia ao Gelo
* R2: Resistencia ao Fogo
* L1: Resistência Sagrada
* R2: Resistencia a Relâmpago
* Select: Resistencia a Trevas
* Start: Resistencia a Petrificação
* Familiar Cloak: Poção de Luck
* X no Inventário pra selecionar a Faerie

## Sword #5

![Sword](https://user-images.githubusercontent.com/36766041/180565051-118eb564-cf4f-41b6-b012-e09a3e967479.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Quadrado: Ataque Giratorio
* Triangulo: Sword Brothers (Sem Consumo de MP)
* Familiar Cloak: Poção de STR
* Ganhos de Dano Original
* R1 no Inventário pra selecionar o Sword

## Pixie (Fairy) #6

![Pixie](https://user-images.githubusercontent.com/36766041/180565721-7a2a27de-da53-41cb-be8e-8c9b99912257.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Vira automaticamente pra Esquerda e Direita
* Quadrado: Usa Potion
* X: Usa Antivenom
* Circulo: Usa Hammer
* Triangulo: Usa Uncurse
* Quadrado + Circulo: Usa Life Apple
* R1: Resistencia ao Gelo
* R2: Resistencia ao Fogo
* L1: Resistência Sagrada
* R2: Resistencia a Relâmpago
* Select: Resistencia a Trevas
* Start: Resistencia a Petrificação
* DPAD pra Baixo + Quadrado: Senta nos ombros do Alucard
* DPAD pra Cima + Quadrado: Canta (Se estiver sentada)
* Familiar Cloak: Regeneração de 1 HP por segundo caso o HP esteja menor que 30.
* Triangulo no Inventário pra selecionar a Pixie

## Nose Devil (NDemon) #7

![Nose Devil](https://user-images.githubusercontent.com/36766041/180565752-159bd6bd-9f42-467e-a046-0605248ff38c.PNG)

* DPAD: Movimenta o Familiar pela Tela
* Triangulo + DPAD Esquerdo e Direito: Muda o Lado em que o Familiar está olhando
* Quadrado: Ataque Comum Aleatório
* X: Ataque Especial Aleatório (LV 10 ou Superior)
* Familiar Cloak: Regeneração de 1 MP por segundo caso o MP esteja menor que 30.
* Mudança das Habilidades e dos Danos de forma Original
* Start no Inventário pra selecionar o Nose Devil
